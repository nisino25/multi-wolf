    ...
    <body :style="currentTime == 'day' ? 'background-color: #E0B589;' : 'background-color: #363945; color: white'" >
        ...
        <div class="container" v-if="readyToPlay">
          ...
          <div class="nav" style="margin-bottom: 10px;">
            ...
            <i v-if="currentTime == 'day'" class="fa-regular fa-sun"></i>
            <i v-if="currentTime == 'night'" class="fa-solid fa-moon"></i>
            </span>
          ...
          <div class="players-container">
            <template  v-for="(player,index) in sortedObj" :key="index">
              <div class="card"  :style="[!player.alive  ? 'opacity: 0.5 ' : '']" :class="[targetVisible(player) ? 'targeted' : '']"  @click="clickingAPlayer(player)">
                ...
                <i v-if="player.alive"  class="fa-solid fa-user"></i>
                <i v-if="!player.alive" class='fas fa-skull-crossbones' style='font-size:35px;color:red; margin-top: 5px'></i>
  
                <div class="container">
                  <!-- name   -->
                  <span class="name" :style="myPlayer?.name == player.name ? 'color:blue': ''"><i v-if="myPlayer.permanentTarget == player.name && myPlayer.role == 'loudmouth'"  class="fa-solid fa-bullseye" style="margin-right: 2.5px; color: crimson;"></i>{{player.name}}</span>
                  <hr>
          ...
          <div class="controller" v-if="!winner">
              ...
              <template v-if="myPlayer.alive">
                ...
                <template v-else>
                  <div class="button-container">
                    ...
                    <button v-if="canIOpenChat()" @click="showingChat = true" class="purple-blink" style="width: 100px;">Open Chat</button>
    
                    <button v-if="!myPlayer.done && latestData.isNightOver && !latestData.isDayOver" @click="skipVotingAtDay()">Skip</button>
                  </div>
  
  
                  <div v-if=" latestData.isNightOver && !latestData.isDayOver && !latestData.isVotingOver">
                    <span>Pick one for the voting</span>
            ...
            <div>
              <template v-if="currentTime == 'night' && myPlayer.alive">
                ...
                <span v-if="myPlayer.role == 'werewolf' && detailData.length == 1 && currentTime == 'night' ">(Werewolf cannot kill on day1)</span>
                    ...
                    <form onsubmit="event.preventDefault()">
                      ...
                      <span>Currently {{members.length}}/9</span><br>
                       ...
                       <!-- <samp>{{members}}</samp> -->
                        ...
                        <template v-for="(player,index) in members" :key="index">
  ...
  export default {
    ...
    methods:{
      ...
      createARoom(){
        ...
        const tempRoomCode = Math.floor(Math.random() * 90000) + 10000;
        this.skipFlag = false
      ...
      skipVotingAtDay() {
        const confirmation = confirm('Are you sure you want to skip voting?');
        if (!confirmation) return
        
        // If user confirms (confirmation is true), proceed with skipping
        this.myPlayer.target = 'skipping';
        this.confirm();
      },
      confirmCheck(){
      ...
      clickingAPlayer(target){
        ...
        // for the voting
        if(this.currentTime == 'day'){ 
          const confirmation = confirm(`Do you want to vote for ${target.name}'`);
          if(!confirmation) return
        ...
        if(this.myPlayer.team == 'wolves'){
          ...
          const confirmation = confirm(`Do you want to kill ${target.name}?`);
          if (!confirmation) return
        ...
        if(this.myPlayer.role == 'seer'){
          const confirmation = confirm(`Do you want to check ${target.name}'s roll?`);
          if (!confirmation) return
        ...
        if(this.myPlayer.role == 'detective'){
          if(this.myPlayer.target == '' || !this.myPlayer.target){
            const confirmation = confirm(`Selecting ${target.name} as your first target?`);
            if (!confirmation) return
            ...
            const confirmation = confirm(`Selecting ${target.name} as your second target?`);
            if (!confirmation) return
